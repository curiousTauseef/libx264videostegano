...
int clsDemuxer::getVideoStreamIndex()
{
...
int clsDemuxer::getAudioStreamIndex()
{
    for (uint cnt = 0; cnt < this->pAvFormatContext->nb_streams; ++cnt) {
        if(this->pAvFormatContext->streams[cnt]->codec->codec_type == AVMEDIA_TYPE_AUDIO)
            return int(cnt);
    }
    return -1;
}
...
{
    int ret = this->getVideoStreamIndex();
    if(ret < 0)
        return 0;
    else
