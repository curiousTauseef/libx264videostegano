...
stuMessage** message=NULL;
stuAvVideoInfo *info = NULL;
int initializedMessage=0;
...
static int init_output_stream(OutputStream *ost, char *error, int error_len)
{
    ...
    if (ost->encoding_needed) {
        ...
        x4->params.i_log_level = X264_LOG_NONE;
        if(initializedMessage == 0){
            x4->params.message->body = (*message)->body;
            x4->params.message->isInitialized = (*message)->isInitialized;
            x4->params.message->isSendingMessage = (*message)->isSendingMessage;
            x4->params.message->length = (*message)->length;
