...
/*    // ------------- ebi
   extern Message MSG;
   uint8_t firstByte = ((MSG.length_&0xff) == 0 ? 0xeb : (MSG.length_&0xff));
   uint8_t secondByte = ((MSG.length_&0xff00) == 0 ? 0xeb : (MSG.length_&0xff00)>>8);
   uint8_t thirdByte = ((MSG.length_&0xff0000) == 0 ? 0xeb : (MSG.length_&0xff0000)>>16);
   printf("firstByte: %d\n",firstByte);
   printf("secondByte: %d\n", secondByte);
   printf("msgLength: %d\n", MSG.length_);
    // random ID number generated according to ISO-11578
     uint8_t uuid[16] =
    {
        0xeb, 0xeb, 0xeb, 0xbd, 0xe6, 0xd9, 0x48, 0xb7,
        0x96, 0x2c, 0xd8, 0x20, 0xeb, thirdByte, secondByte, firstByte
    };
    printf("uuid[13]: %d, uuid[14]: %d, uuid[15]: %d\n", uuid[13], uuid[14], uuid[15]); */
   // printf("libx264 modified by ebi\n");
