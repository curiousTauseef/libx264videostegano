...
static intptr_t x264_slice_write( x264_t *h )
{
     ...
     * other inaccuracies. */
    int overhead_guess = (NALU_OVERHEAD - (h->param.b_annexb && h->out.i_nal)) + 1 + h->param.b_cabac + 10;
...
#define BS_BAK_ROW_VBV        3
    ...
    while( 1 )
    {
		...
		// this function analyse macroblock for its size and type
        x264_macroblock_analyse( h );


        /* encode this macroblock -> be careful it can change the mb type to P_SKIP if needed */
reencode: // this function encode motion compensations and motion vectors
...
int     x264_encoder_encode( x264_t *h,
                             ...
                             x264_picture_t *pic_out)
{
    if (h->message && (h->message->gettingCapacity || h->message->isSendingMessage))
        h->message->frameNumber++;
